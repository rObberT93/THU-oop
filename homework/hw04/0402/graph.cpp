...
void Graph::InsertOperation(const string &node_name, const string &operation_type, 
    ...
    if(operation_type == "+") {
        Node* p = new operatorAdd(node_name, nodes_[input1], nodes_[input2]);
        nodes_.insert(node_name, p);
    }
    else if(operation_type == "-") {
        Node* p = new operatorSub(node_name, nodes_[input1], nodes_[input2]);
        nodes_.insert(node_name, p);
    }
}
...
void Graph::Eval(const string &node_name, const int &argument_num, 
    ...
    for(int k = 0; k < argument_num; k++) {
        PlaceholderNode *pn = dynamic_cast<PlaceholderNode*>(nodes_[argument_name_list_[k]]);
        if(nodes_[argument_name_list_[k]] != nullptr) {
            pn->set_val(argument_value_list_[k]);
        }
    }
